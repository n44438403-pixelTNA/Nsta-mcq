...
export const MarksheetCard: React.FC<Props> = ({
  ...
  const devName = settings?.footerText || "shivangi";
      ...
      <div className="bg-white rounded-2xl p-6 shadow-xl border border-slate-200 mt-6 break-inside-avoid" data-export-hide="true">
        ...
        <div className="space-y-4">
          {topics.map((topic, i) => {
            ...
            const topicQuickNotes = Array.from(new Set(topicQs.map((q) => q.quickNote).filter(Boolean)));
            const topicDeepNotes = Array.from(new Set(topicQs.map((q) => q.deepNote).filter(Boolean)));
            // Legacy fallback
            const legacyNotes = Array.from(new Set(topicQs.map((q) => q.note).filter(Boolean)));

            const hasQuickNotes = topicQuickNotes.length > 0;
            const hasDeepNotes = topicDeepNotes.length > 0;
            const hasLegacyNotes = !hasQuickNotes && !hasDeepNotes && legacyNotes.length > 0;
            ...
            return (
              ...
              >
                ...
                {hasQuickNotes && (
                  <div className="p-4 bg-yellow-50/50 border-b border-slate-100">
                    <p className="text-[10px] font-bold text-yellow-600 uppercase tracking-widest mb-3 flex items-center gap-1">
                      <Zap size={12} /> Quick Revision
                    </p>
                    <div className="space-y-3">
                      {topicQuickNotes.map((note, nIdx) => (
                        <div
                          key={nIdx}
                          className="text-xs text-slate-700 leading-relaxed bg-white p-3 rounded-xl border border-yellow-100 shadow-sm"
                          dangerouslySetInnerHTML={{
                            __html: renderMathInHtml(note?.replace(/\n/g, '<br/>') || ''),
                          }}
                        />
                      ))}
                    </div>
                ...
                {hasDeepNotes && (
                  <div className="p-4 bg-blue-50/30">
                    <p className="text-[10px] font-bold text-blue-600 uppercase tracking-widest mb-3 flex items-center gap-1">
                      <BookOpen size={12} /> Deep Revision
                    </p>
                    <div className="space-y-3">
                      {topicDeepNotes.map((note, nIdx) => (
                        <div
                          key={nIdx}
                ...
                {hasLegacyNotes && (
                  <div className="p-4 bg-slate-50/80">
                    <p className="text-[10px] font-bold text-slate-500 uppercase tracking-widest mb-3 flex items-center gap-1">
                      <BookOpen size={12} /> Topic Notes
                    </p>
                    <div className="space-y-3">
                      {legacyNotes.map((note, nIdx) => (
                        <div
                          key={nIdx}
                          className="text-xs text-slate-700 leading-relaxed bg-white p-3 rounded-xl border border-slate-200 shadow-sm"
                          dangerouslySetInnerHTML={{
                            __html: renderMathInHtml(note?.replace(/\n/g, '<br/>') || ''),
                          }}
                        />
                ...
                {!hasQuickNotes && !hasDeepNotes && !hasLegacyNotes && status !== "STRONG" && (
