...
export const Auth: React.FC<Props> = ({ onLogin, onSkip, logActivity }) => {
  ...
  const handleCompleteSignup = async () => {
      try {
          ...
          const derivedName = formData.email.split('@')[0].replace(/[._\-]/g, ' ').replace(/\b\w/g, c => c.toUpperCase()) || 'Student';
      ...
      <div className="bg-white p-8 rounded-3xl shadow-xl w-full max-w-md border border-slate-200 relative overflow-hidden">
        ...
        {view !== 'HOME' && (
            <h2 className="text-xl font-bold text-slate-800 mb-6 flex items-center gap-2 relative z-10">
              ...
              <span className="flex-1">
                {view === 'LOGIN' && 'Login with Email'}
                {view === 'SIGNUP' && 'Create Account'}
                {view === 'RECOVERY' && 'Request Login'}
                {view === 'ADMIN' && (showAdminVerify ? 'Admin Verification' : 'Admin Login')}
              </span>

              {view === 'LOGIN' && <SpeakButton text="Login with Email. Enter your registered email and password." className="text-blue-600 hover:bg-blue-50" />}
        ...
        {view === 'HOME' && (
            <div className="space-y-3 relative z-10 animate-in fade-in mt-8">
                 ...
                 <button type="button" onClick={() => setView('SIGNUP')} className="w-full bg-gradient-to-br from-blue-600 to-indigo-600 hover:from-blue-700 hover:to-indigo-700 text-white font-bold py-4 rounded-[2rem] flex items-center justify-center gap-3 transition-all active:scale-95 shadow-lg">
                     <UserPlus size={20} />
                     Create Account
                 </button>

                 {/* LOGIN (Email) */}
                 <button type="button" onClick={() => setView('LOGIN')} className="w-full bg-white border-2 border-slate-200 hover:border-blue-500 hover:bg-blue-50 text-slate-800 font-bold py-4 rounded-[2rem] flex items-center justify-center gap-3 transition-all active:scale-95 shadow-sm">
                 ...
                 <div className="relative pt-2">
                     ...
                     <div className="relative flex justify-center text-[10px] uppercase"><span className="bg-white px-2 text-slate-500 font-bold">Or</span></div>
                 </div>

                 {onSkip && (
                     <button type="button" onClick={onSkip} className="w-full bg-slate-50 border border-slate-200 hover:bg-slate-100 text-slate-500 font-bold py-3 rounded-2xl flex items-center justify-center gap-2 transition-all active:scale-95 text-sm">
                         Skip for now
                     </button>
                 )}

                 <p className="text-center text-[10px] text-slate-400 font-medium px-4">
                     MCQ practice requires an account.
                 </p>
            </div>
        )}
        ...
        {view !== 'HOME' && (
            <form onSubmit={handleSubmit} className="space-y-4 relative z-10">
              ...
              {view === 'LOGIN' && (
                  <>
                     <div className="space-y-1.5"><label className="text-xs font-bold text-slate-600 uppercase">Email Address</label><input name="id" type="email" placeholder="Enter your email" value={formData.id} onChange={handleChange} className="w-full px-4 py-3 border border-slate-200 rounded-xl font-bold" /></div>
                     <div className="space-y-1.5">
                         <label className="text-xs font-bold text-slate-600 uppercase">Password</label>
                         <div className="relative">
                             <input name="password" type={showPassword ? "text" : "password"} placeholder="Enter your password" value={formData.password} onChange={handleChange} className="w-full px-4 py-3 border border-slate-200 rounded-xl font-bold pr-10" />
