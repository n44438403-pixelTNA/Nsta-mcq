...
export const getUserData = async (userId: string) => {
    try {
        ...
        // Try Firestore FIRST (Primary source of truth for role/profile)
...
export const getUserByEmail = async (email: string) => {
    try {
        ...
        if (!querySnapshot.empty) {
            let coreData = querySnapshot.docs[0].data();
            if (coreData.email === ADMIN_EMAIL && coreData.role !== 'ADMIN') {
                coreData.role = 'ADMIN';
                await setDoc(doc(db, "users", coreData.id), { role: 'ADMIN' }, { merge: true });
            }
