...
import { getChapterData, saveChapterData, checkFirebaseConnection, saveTestResult, saveUserToLive, updateUserStatus, getUserData, subscribeToSettings, auth, savePublicActivity, saveUserHistory, getUserSavedNotes, getUserByEmail, updateUserUID } from './firebase';
import { storage } from './utils/storage';
import { recalculateSubscriptionStatus, addSubscription } from './utils/subscriptionUtils';
import { signInAnonymously, GoogleAuthProvider, signInWithPopup, setPersistence, browserLocalPersistence } from 'firebase/auth';
...
const App: React.FC = () => {
  ...
  // MCQ AUTH MODAL STATE (for guest users trying MCQ)
  const [showMCQAuthModal, setShowMCQAuthModal] = useState(false);
  ...
  useEffect(() => {
    ...
    if (loggedInUserStr) {
      try {
        ...
        // Don't restore guest users — they should see the Auth page on each fresh load
    ...
    // No stored user — Auth page will show automatically (state.user is null)
  }, []);
  ...
  const performLogout = () => {
    ...
    localStorage.removeItem('nst_current_user');
    localStorage.removeItem('nst_user_history');
    setState(prev => ({
        ...prev,
        user: null,
        originalAdmin: null,
  ...
  const handleGoogleConnect = async () => {
    try {
        ...
        if (appUser) {
            ...
            logActivity("GOOGLE_CONNECT", "Connected Google Account", appUser);
            handleLogin(appUser);
        } else {
            // Merge current user data with Google UID
            if (state.user) {
                const merged = { ...state.user, id: firebaseUser.uid, email: firebaseUser.email || state.user.email, provider: 'google' };
                await saveUserToLive(merged);
                handleLogin(merged);
            }
        }
    } catch (err: any) {
        console.error("Google Connect Error:", err);
        setAlertConfig({ isOpen: true, message: "Google connect failed: " + (err.message || "Please try again.") });
    }
  };
  ...
  const handleContentGeneration = async (type: ContentType, count?: number, forcePay: boolean = false, specificContent?: any) => {
    ...
    // --- GUEST MCQ GATE: Require login for MCQ content ---
    const isMCQType = ['MCQ_SIMPLE', 'MCQ_PRACTICE', 'MCQ_TEST', 'MCQ_ANALYSIS'].includes(type);
    if (isMCQType && state.user.id.startsWith('guest_')) {
        setShowMCQAuthModal(true);
        setLoadingContentType(null as any);
        return;
    }
  ...
  return (
    ...
    <div className="min-h-screen flex flex-col bg-white font-sans relative pt-[env(safe-area-inset-top,24px)] pb-[env(safe-area-inset-bottom,0px)]">
      ...
      {/* MCQ AUTH MODAL — shown to guest users who try to access MCQ */}
      {showMCQAuthModal && (
          <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-[9999] p-4">
              <div className="bg-white rounded-3xl p-8 w-full max-w-sm shadow-2xl text-center animate-in zoom-in duration-200">
                  <div className="w-16 h-16 bg-blue-100 rounded-full flex items-center justify-center mx-auto mb-4">
                      <span className="text-3xl">🎯</span>
                  </div>
                  <h2 className="text-2xl font-black text-slate-800 mb-2">Account Required</h2>
                  <p className="text-slate-500 text-sm mb-6">MCQ practice ke liye account banana zaroori hai. Apna account banao ya login karo.</p>
                  <div className="space-y-3">
                      <button
                          onClick={() => { setShowMCQAuthModal(false); setState(prev => ({...prev, user: null, view: 'STUDENT_DASHBOARD'})); }}
                          className="w-full bg-blue-600 hover:bg-blue-700 text-white font-bold py-3.5 rounded-2xl transition-all"
                      >
                          Create Account / Login
                      </button>
                      <button
                          onClick={() => setShowMCQAuthModal(false)}
      ...
      <main className={`flex-1 w-full max-w-6xl mx-auto ${isFullScreen ? 'p-0' : 'p-4 mb-8'}`}>
        {!state.user ? (
            <Auth onLogin={handleLogin} onSkip={handleSkipAuth} logActivity={logActivity} />
        ) : state.view === 'AUTH' ? (
            <Auth onLogin={handleLogin} onSkip={handleSkipAuth} logActivity={logActivity} />
        ) : (
            <ErrorBoundary>
